...
| 双通道（上海） | ✅ 已入库 | 上海首批名单 96 条 |
| 挂网价（湖南） | ✅ 已入库 | price_history 湖南 337 条（最小制剂价格口径） |
| 医保支付标准（上海） | ✅ 已入库 | 28 条品种行（目录 472 条目中 152 条含金额） |
...
## 二、已完成写入（2026-09-17 补跑）

以下命令已于 2026-09-17 在本地执行完成：

```
上海双通道：parsed 96 / stored 96 / dual_channel_total 680
上海支付标准：with_price 152 / products_written 28 / price_history_total 337
湖南价格与双通道：price_products 337 / price_history_total 365 / dual_channel_total 680
快照导出：drugs.json 1,601 行
```
...
2. **品种覆盖率是主要瓶颈**：湖南价格表 12.4 万行中仅 337 个品种能回挂
